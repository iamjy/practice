...
static dev_t        dev;
static unsigned int dev_cnt = 1;    /* Number of dev_t needed */
...
{
#if 1
    if (alloc_chrdev_region(&dev, 0, dev_cnt, CHAR_DEV_NAME) < 0) {
		pr_err("Failed to allocate character device region\n");
		return -1;
	}
#else /* Either of these will work */
    dev = MKDEV(my_major, my_minor);
	if (register_chrdev_region(dev, dev_cnt, CHAR_DEV_NAME) < 0) {
