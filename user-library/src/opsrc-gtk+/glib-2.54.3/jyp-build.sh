...
export PKG_CONFIG_PATH=/usr/local/lib/pkgconfig/arm:/usr/lib/x86_64-linux-gnu/pkgconfig
...
#: <<'END'
cp arm-linux.cache arm.cache
echo "./configure --cache-file=arm.cache --enable-libmount=no --with-threads=posix --prefix=${TARGET_PATH} --host=${TARGET_MACHINE} --target=${TARGET_MACHINE}"
./configure --cache-file=arm.cache --enable-libmount=no --with-threads=posix --prefix=${TARGET_PATH} --host=${TARGET_MACHINE} --target=${TARGET_MACHINE}
make
make install
#END
...
export PKG_CONFIG_PATH=/usr/lib/x86_64-linux-gnu/pkgconfig:/usr/local/lib/pkgconfig/arm
